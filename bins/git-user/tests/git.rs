...
impl GitRepo {
    ...
    /// Runs the user command in the repository.
    pub fn run_user(&self, args: &[&str]) -> Output {
        let path = self.path.path().join("users.json").display().to_string();
        self.run_cmd(env!("CARGO_BIN_EXE_git-user"), &[&["--config", &path], args].concat())
    }
